<template>
  <div class="p-6 space-y-6" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
    <!-- Smart Lock Screen for Free Plan Users -->
    <div
      v-if="!can('allow_analytics')"
      class="bg-white p-8 md:p-14 rounded-[2.5rem] border border-orange-100 shadow-sm text-center flex flex-col items-center justify-center relative overflow-hidden"
    >
      <div
        class="w-16 h-16 bg-gradient-to-tr from-orange-500 to-amber-400 text-white rounded-3xl flex items-center justify-center text-3xl shadow-lg shadow-orange-500/25 mb-4"
      >
        📊
      </div>
      <span
        class="inline-block bg-orange-100 text-orange-700 text-xs font-black px-4 py-1.5 rounded-full mb-3"
      >
        {{ $t("plans.exclusive_badge") }}
      </span>
      <h2 class="text-2xl md:text-3xl font-black text-slate-900 mb-2">
        {{ $t("plans.sales_lock_title") }}
      </h2>
      <p
        class="text-slate-500 text-sm md:text-base max-w-lg mb-8 leading-relaxed font-medium"
      >
        {{ $t("plans.sales_lock_desc") }}
      </p>

      <button
        type="button"
        @click="showUpgradeModal = true"
        class="bg-gradient-to-r from-orange-600 to-amber-500 hover:from-orange-500 hover:to-amber-400 text-white font-black py-4 px-8 rounded-2xl shadow-xl shadow-orange-500/25 transition-transform active:scale-95 text-base cursor-pointer"
      >
        ⚡ {{ $t("plans.sales_lock_btn") }}
      </button>

      <!-- Upgrade Modal -->
      <UpgradeModal
        :isOpen="showUpgradeModal"
        :title="$t('plans.sales_modal_title')"
        :badgeText="$t('plans.basic_badge_short')"
        :message="$t('plans.sales_modal_msg')"
        @close="showUpgradeModal = false"
      />
    </div>

    <!-- المحتوى الفعلي للمشتركين -->
    <template v-else>
...
const fetchSales = async () => {
  if (!userId.value || !can('allow_analytics')) return;
  loading.value = true;
